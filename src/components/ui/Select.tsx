...
export const Select: React.FC<SelectProps> = ({
  ...
  const DropdownMenu = () => (
    <div className={`absolute z-50 overflow-hidden bg-white border border-slate-200 rounded-lg shadow-dropdown animate-fade-in ${variant === 'header' ? 'top-full left-[-1px] mt-2 w-[240px]' : variant === 'minimal' ? 'top-full right-0 mt-2 w-56' : 'top-full left-0 mt-2 w-full'}`}>
      {searchable && (
      ...
      <div className="max-h-[280px] overflow-y-auto py-1 custom-scrollbar">
        {filteredOptions.length > 0 ? (
          filteredOptions.map((option) => (
            <div
              key={option.value}
              className={`px-4 py-2.5 text-sm font-medium flex items-center justify-between hover:bg-slate-50 transition-colors cursor-pointer group ${
      ...
      >
        ...
        >
          <div className="flex flex-col pointer-events-none">
              <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider mb-1 transition-colors group-hover:text-primary-500">{label}</span>
  ...
  if (variant === 'minimal') {
    return (
      <div ref={containerRef} className={`relative ${className}`}>
        <div
          className={`flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-600 hover:text-slate-900 transition-colors cursor-pointer ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
          onClick={() => !disabled && setIsOpen(!isOpen)}
        >
...
<div
  ...
  } ${isOpen ? 'ring-2 ring-primary-500/20 bg-white border-primary-500' : ''}`}
  onClick={() => !disabled && setIsOpen(!isOpen)}
>
